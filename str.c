                           ...
                           unsigned int len)
{
  ...
  {
    str_free(p_str);
    s_setbuf(p_str, asev_sysutil_malloc(buf_needed));
    p_str->alloc_bytes = buf_needed;
  }
  asev_sysutil_memcpy(p_str->p_buf, p_src, len);
  p_str->p_buf[len] = '\0';
  p_str->len = len;
}
                            ...
                            unsigned int len)
{
  ...
  if (buf_needed > p_str->alloc_bytes)
  {
    p_str->p_buf = asev_sysutil_realloc(p_str->p_buf, buf_needed);
    p_str->alloc_bytes = buf_needed;
  }
  asev_sysutil_memcpy(p_str->p_buf + p_str->len, p_src, len);
...
{
  unsigned int len = asev_sysutil_strlen(p_src);
...
{
  return asev_sysutil_strdup(str_getbuf(p_str));
}
...
{
  str_alloc_text(p_str, asev_sysutil_ulong_to_str(the_long));
}

void
str_alloc_filesize_t(struct mystr* p_str, filesize_t the_filesize)
{
  str_alloc_text(p_str, asev_sysutil_filesize_t_to_str(the_filesize));
}
...
str_reserve(struct mystr* p_str, unsigned int res_len)
{
  ...
  {
    p_str->p_buf = asev_sysutil_realloc(p_str->p_buf, res_len);
                   ...
                   const char* p_buf2, unsigned int buf2_len)
{
  ...
  retval = asev_sysutil_memcmp(p_buf1, p_buf2, minlen);
...
{
  unsigned int cmplen = asev_sysutil_strlen(p_text);
...
{
  str_append_text(p_str, asev_sysutil_ulong_to_str(the_ulong));
}

void
str_append_filesize_t(struct mystr* p_str, filesize_t the_filesize)
{
  str_append_text(p_str, asev_sysutil_filesize_t_to_str(the_filesize));
}

void
str_append_double(struct mystr* p_str, double the_double)
{
  str_append_text(p_str, asev_sysutil_double_to_str(the_double));
}
...
str_upper(struct mystr* p_str)
{
  ...
  {
    p_str->p_buf[i] = (char) asev_sysutil_toupper(p_str->p_buf[i]);
  }
}
                      ...
                      const char* p_text, int is_reverse)
{
  ...
  unsigned int indexx;
  unsigned int search_len = asev_sysutil_strlen(p_text);
  if (is_reverse)
  {
...
str_locate_chars(const struct mystr* p_str, const char* p_chars)
{
  ...
  unsigned int num_chars = asev_sysutil_strlen(p_chars);
  unsigned int i = 0;
  retval.found = 0;
  retval.char_found = 0;
  retval.index = 0;
...
str_locate_text(const struct mystr* p_str, const char* p_text)
{
  struct str_locate_result retval;
  unsigned int i;
  unsigned int text_len = asev_sysutil_strlen(p_text);
  retval.found = 0;
  retval.char_found = 0;
  retval.index = 0;
  ...
  {
    if (asev_sysutil_memcmp(p_str->p_buf + i, p_text, text_len) == 0)
    {
      retval.found = 1;
      retval.index = i;
      return retval;
...
str_contains_space(const struct mystr* p_str)
{
  ...
  for (i=0; i < p_str->len; i++)
  {
    if (asev_sysutil_isspace(p_str->p_buf[i]))
    {
      return 1;
    }
  }
...
str_contains_unprintable(const struct mystr* p_str)
{
  ...
  for (i=0; i < p_str->len; i++)
  {
    if (!asev_sysutil_isprint(p_str->p_buf[i]))
    {
      return 1;
    }
  }
...
{
  return asev_sysutil_atoi(str_getbuf(p_str));
}

filesize_t
str_a_to_filesize_t(const struct mystr* p_str)
{
  return asev_sysutil_a_to_filesize_t(str_getbuf(p_str));
}

unsigned int
str_octal_to_uint(const struct mystr* p_str)
{
  return asev_sysutil_octal_to_uint(str_getbuf(p_str));
}
